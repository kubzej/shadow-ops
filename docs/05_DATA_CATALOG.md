...
- `MissionCategory` rozšířena o `counter`.
- Counter-Ops používají interní target (`__counter__`) generovaný engine funkcí `generateCounterOp()`.
- Rival event katalog (engine-level typy):
  - `asset_compromise`
  - `intel_theft`
  - `sabotage`
  - `agent_recruitment`
