...
export function generateBlackMarketOffer(): BlackMarket {
  ...
  const selected: Equipment[] = pickRandomN(eligibleEquipment, LISTING_COUNT, rng)
...
function _specialAgentListing(rng: () => number): BlackMarketListing {
  const eliteDivisions: DivisionId[] = ['blackops', 'extraction', 'cyber', 'sabotage']
  const division = pickRandom(eliteDivisions, rng)
  // Represent as a sentinel equipment id (handled specially by the UI)
  return {
    equipmentId:   `__agent__${division}__specialist`,
    costShadow:    25,
    costInfluence: 15,
    costMoney:     0,
  }
...
export function parseSpecialAgentListing(equipmentId: string): {
  ...
  return { division: match[1] as DivisionId, rank: match[2] as 'specialist' | 'veteran' }
}
